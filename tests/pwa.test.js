...
function serviceWorker(){
 ...
 const context={URL,caches:{open:async()=>cache,keys:async()=>['undertone-v3','undertone-v11','undertone-v13','undertone-v14','unrelated-cache','undertone-library-v1'],delete:async key=>deleted.push(key)},fetch:async req=>{network.push(req);return 'network';},self:{location:{origin:'https://undertone.test'},clients:{claim:async()=>{}},addEventListener:(type,fn)=>handlers[type]=fn}};
...
test('activation removes only obsolete Undertone caches',async()=>{
 const sw=serviceWorker();let ready;sw.handlers.activate({waitUntil:p=>ready=p});await ready;assert.deepEqual(sw.deleted,['undertone-v3','undertone-v11','undertone-v13']);
});
