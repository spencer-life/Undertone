...
| Motion | Slow shared drift with a low maximum speed | Nonlinear speed range plus restrained music-reactive motion | Make the scenes visibly alive while keeping reduced-motion and calm-product constraints |
...
produce a still composition. While audio is playing, one shared spectral envelope
can modestly accelerate scene time and shape scene-specific motion; this is reactive
visual accompaniment, not a neurological measurement or a literal binaural-frequency
visualization. Droplet sprites and the defocused background are cached by size, seed
and theme.
